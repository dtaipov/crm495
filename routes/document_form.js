...
module.exports = {
    ...
    edit_save: (req, res, next) => {
        const agentId = parseInt(req.body.agent_id);
        db.documents.total([req.user.id, req.user.login])
        .then((totalResult) => {
            const maxDemoDocuments = 50;
            if (totalResult.count >= maxDemoDocuments) {
                throw `Cannot create more than ${maxDemoDocuments} documents for the Demo user`;
            }})
        .then( () =>
            db.documents.edit(
              {
